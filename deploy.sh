...
rm -rf bin/icebox$EXE
sbcl --disable-debugger \
     --load cl-icebox.asd \
     --eval "(ql:quickload 'cl-icebox)" \
     --eval "(asdf:make :cl-icebox)"
...
then
    ...
    mkdir -p out/tmp
    cp ./bin/icebox.exe ./out/tmp/icebox.exe
    ...
    bin/warp-packer.exe --arch windows-x64 --input_dir ./out/tmp/ --exec icebox.exe --output ./out/tmp/icebox-warp.exe
    bin/rh/rh.exe -open ./out/tmp/icebox-warp.exe -save ./out/icebox.exe -action addskip -res ./resources/iconfile.ico -mask ICONGROUP,MAINICON
    "$EB" /subsystem:windows ./out/icebox.exe
    rm -rf out/tmp
fi

if [[ "$OS" == "osx" ]]
then
    export OSX_APP_DIR=out/icebox.app/Contents
    mkdir -p $OSX_APP_DIR
    cp ./resources/Info.plist $OSX_APP_DIR/
    mkdir -p $OSX_APP_DIR/MacOS
    cp ./bin/icebox ./bin/tclkit-gui $OSX_APP_DIR/MacOS
    mkdir -p $OSX_APP_DIR/Resources
    cp ./resources/iconfile.icns $OSX_APP_DIR/Resources
    cd out && zip -r -9 icebox$APP.zip icebox.app && cd ..
    rm -rf out/icebox.app
fi

if [[ "$OS" == "linux" ]]
then
    export LINUX_APP_DIR=out/icebox.AppDir
    mkdir -p $LINUX_APP_DIR
    cp resources/AppRun $LINUX_APP_DIR/AppRun
    cp resources/icebox.desktop $LINUX_APP_DIR/
    cp resources/iconfile.svg $LINUX_APP_DIR/
    mkdir -p $LINUX_APP_DIR/usr/bin
    cp bin/icebox $LINUX_APP_DIR/usr/bin/
    ...
    cd out && ../bin/appimagetool icebox.AppDir icebox.AppImage && cd ..

    rm -rf out/icebox.AppDir
fi
